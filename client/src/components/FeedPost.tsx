...
export default function FeedPost({ post }: { post: IFeedPost }) {
    return (
        <div className="p-6 space-y-4">
            ...
            <p>{post.body}</p>
            {post.quotedId && (
                <p className="border rounded-md px-4 py-3 text-sm dark:border-gray-700">💬 Expand to view the quoted post.</p>
            )}
            <div className="flex items-center gap-4 text-sm">
                <p><span className="font-bold text-black dark:text-white">{post.score}</span> points</p>
                <p><span className="font-bold text-black dark:text-white">{post.replies}</span> comments</p>
            </div>
        </div>
    );
};